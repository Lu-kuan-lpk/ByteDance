...
public class UploadActivity extends AppCompatActivity {
    ...
    //定义一个retrofit变量，其中的baseurl和转换模式都给定
    ...
    private void submit() {
        ...
        MultipartBody.Part picPart = MultipartBody.Part.createFormData("image",
                "cover.png",
                RequestBody.create(MediaType.parse("multipart/form-data"),
                        coverImageData));

        MultipartBody.Part namePart =MultipartBody.Part.createFormData("from",Constants.USER_NAME);
        MultipartBody.Part toPart =MultipartBody.Part.createFormData("to",to);
        MultipartBody.Part conPart =MultipartBody.Part.createFormData("content",content);

        Call<UploadResponse> upload = service.submitMessage(
                Constants.STUDENT_ID,"",namePart,toPart,
                conPart,picPart,"WkpVLWJ5dGVkYW5jZS1hbmRyb2lk");

        Log.d(TAG, "onResponse: enqueue");
            ...
            public void onResponse(Call<UploadResponse> call, Response<UploadResponse> response) {
                if(!response.isSuccessful()){
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(UploadActivity.this,"transfer the message failed!",Toast.LENGTH_SHORT).show();
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(UploadActivity.this,"transfer the message success!",Toast.LENGTH_SHORT).show();
