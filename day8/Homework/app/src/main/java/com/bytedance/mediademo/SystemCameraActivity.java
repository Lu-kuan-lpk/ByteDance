...
public class SystemCameraActivity extends AppCompatActivity {
    ...
    //最后会得到一个 data的返回intent，我们从中提取数据
    @Override
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_TAKE_PHOTO && resultCode == RESULT_OK) {
            ...
            options.inJustDecodeBounds=true;//只解码图片的宽高的信息
            BitmapFactory.decodeFile(takeImagePath,options);
            int photoWidth = options.outWidth;
            int photoHeight = options.outHeight;

            //计算缩放的比例
            int scalef = Math.min(photoWidth/targetWidth,photoHeight/targetHeight);
            options.inJustDecodeBounds=false;
            options.inSampleSize = scalef;
