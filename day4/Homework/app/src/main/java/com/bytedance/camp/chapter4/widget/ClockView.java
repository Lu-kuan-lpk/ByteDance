...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
         ...
         */
        //canvas.drawText(""+hour,centerX,centerY,numberPaint);
        ...
        int MiniuteDegree = minute*6-90;
        endX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(MiniuteDegree)));
        endY = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(MiniuteDegree)));
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,endX,endY,needlePaint);

        int SecDegree = second*6-90;
        endX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(SecDegree)));
        endY = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(SecDegree)));
